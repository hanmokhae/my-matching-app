...
- Name: matching.spec.ts >> 시니어 일자리 매칭 시스템 자동 테스트 >> 엣지 시나리오: 매칭 결과가 없을 때 빈 화면이 잘 처리되는지 확인
- Location: tests\matching.spec.ts:44:9
...
Locator: locator('text=담당자가 직접 연락드리니 잠시만 기다려 주세요')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=담당자가 직접 연락드리니 잠시만 기다려 주세요')
